...
## PR 23 follow-up — real Canvas submit adapter (live-capable)
- [auto] `DirectCanvasSubmitClient` POSTs an online_url (artifact link) or online_text_entry submission with the bearer token to `/courses/:c/assignments/:a/submissions`, throws on non-2xx, and verifies by reading `submissions/self` (only treats a non-'unsubmitted' workflow_state as verified). `createCanvasSubmitClient` picks direct-vs-unconfigured from config. Wired through the existing PR 23 gate (approval + verify-after-write + idempotent). Tested with mock fetch.
- [live-capable but gated] With CANVAS_BASE_URL + CANVAS_API_TOKEN, submission can now actually reach Canvas — but only for an assignment that hit review_ready AND either has exact-version approval (require_review) or policy=auto + AUTO_SUBMIT. Nothing submits by default.
